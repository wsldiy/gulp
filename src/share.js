...
// $('body').addClass('videoBg').css('background-color', '#000');
...
var flag = true;
var player, player2, player3, playing;
if (isAndroid) {
  try {
  ...
  //     bridge.callHandler('onPostSystemType', function (response) {
  //       console.log('收到oc传过来的sdk类型:' + JSON.stringify(response));
  //       iosSdk = response.systemType;
  //       getIosUid(iosSdk)
  //     })
  //   });
} else {
...
function entrance() {
  var options1 = {
    ...
    promotionNetwork: 1,
    uid: cookie_uid,
    hdggAdvInfoId: getUrlParam('hdggAdvInfoId'),
    advLayout: getUrlParam('advLayout')
  };
  $.ajax({
    type: "POST",
    url: baseUrl + 'bhideo/statistics/shareBack',
    data: JSON.stringify(options1),
    async: false,
    headers: {'Content-Type': 'application/json'},
    success: function (res) {
      console.log('分享回流汇报', res)
    }
  });
  //广告入口 获取互动广告信息
  var options2 = {
    ...
    promotionNetwork: 1,
    uid: cookie_uid,
    advInfoId: getUrlParam('hdggAdvInfoId'),
    advLayout: getUrlParam('advLayout')
  };
  $.ajax({
    type: "POST",
    url: baseUrl + 'entrance/shareVideo',
    ...
    success: function (res) {
      ...
      if (res.code == 0) {
        ...
        // console.log(videoList.videoInfoList[0].firstFrame);
        $('body').append('<div style="display: none;" id="shareIcon"><img style="width: 300px;height: 300px;" src="' + videoList.videoInfoList[0].firstFrame
            + '"></div>')
        // $('#shareIcon img').attr('src', videoList.videoInfoList[0].firstFrame)
        boFang()
      }
    }
  });
